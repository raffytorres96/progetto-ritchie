...
1. il testing del vostro codice (unit test con [JUnit](https://junit.org/)) e l'analisi dello stesso con strumenti di quality assurance ([Checkstyle](https://checkstyle.org/), [Spotbugs](https://spotbugs.github.io),[PMD](https://docs.pmd-code.org/latest/));
...
- entrare nella pagina delle impostazioni del proprio profilo GitHub (click sull'immagine di profilo in alto a destra, poi click sulla voce *"Settings"* nel menù a tendina che compare);
  ![GuidaDelloStudente_ritchie_1](./img/GuidaDelloStudente_ritchie_1.jpg)
...
- entrare nella sezione delle impostazioni per sviluppatori, *"Developer Settings"*, facendo click sul relativo pulsante nella barra laterale;
  ![GuidaDelloStudente_ritchie_2](./img/GuidaDelloStudente_ritchie_2.jpg)
...
- selezionare *"Personal access tokens"* nella barra laterale, quindi la sotto-voce *"Tokens (classic)"*. Successivamente fare click sul bottone *"Generate new token"* in alto a destra, quindi su *"Generate new token (classic)"*;
  ![GuidaDelloStudente_ritchie_3](./img/GuidaDelloStudente_ritchie_3.jpg)
...
- selezionare gli ambiti di validità del token (*"Scopes"*). Per consentire al processo Docker di caricare un'immagine su GitHub Packages, il set minimale di scope da abilitare è il seguente (vedi figura):
  ...
  ![GuidaDelloStudente_ritchie_4](./img/GuidaDelloStudente_ritchie_4.jpg)
...
- recarsi sulla pagina principale del repository e fare click sull'icona *"Settings"* (ultima tab in alto a destra);
  **N.B.**: solo l'amministratore visualizza questa tab!
  ![GuidaDelloStudente_ritchie_5](./img/GuidaDelloStudente_ritchie_5.jpg)
...
(vedi freccia rossa in figura); ![GuidaDelloStudente_ritchie_6](./img/GuidaDelloStudente_ritchie_6.jpg)
N.B.: il package sarà visibile solo dopo che GitHub Actions avrà completato con successo la prima build del progetto;
3. cliccare sul link "Package settings", presente nella pagina dedicata al package come ultima voce nella barra laterale destra (vedi freccia rossa in figura); ![GuidaDelloStudente_ritchie_7](./img/GuidaDelloStudente_ritchie_7.jpg)
...
![GuidaDelloStudente_ritchie_9](./img/GuidaDelloStudente_ritchie_9.jpg)
...
- Lasciando invariate le impostazioni di default (`branch` e `event`) nella finestra a comparsa, fare click su `Copy status badge Markdown`;
  ![GuidaDelloStudente_ritchie_10](./img/GuidaDelloStudente_ritchie_10.jpg)
...
- Dopo l’esecuzione dei test è possibile lanciare gli strumenti di **Quality Assurance** (Checkstyle, Spotbugs e PMD) per assicurarsi di aver scritto codice di qualità. Leggere la sezione *Controlli di Qualità* per ulteriori informazioni.
